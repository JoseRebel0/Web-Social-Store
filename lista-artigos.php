...
if (!empty($searchQuery)) {
    $sql .= " WHERE designacao_artigo LIKE '%$searchQuery%' OR descricao LIKE '%$searchQuery%'";
}
?>
...
<div class="page">
                    ...
                    <div class="card-body">
                        <div id="table-default" class="table-responsive">
                            <table class="table">
                                <thead>
                                    <tr>
                                    <th><button class="table-sort" data-sort="sort-nome">Designação</button></th>
                                    <th><button class="table-sort" data-sort="sort-observacao">Descrição</button></th>
    ...
    if (mysqli_num_rows($result) > 0) {
        while ($row = mysqli_fetch_assoc($result)) {
            echo "<tr>";
            echo "<td class='sort-nome'>" . htmlspecialchars($row['designacao_artigo']) . "</td>";
            echo "<td class='sort-observacao'>" . htmlspecialchars($row['descricao']) . "</td>";

            echo '<td><div class="delete">
            <a href="edit-artigo.php?id_artigo=' . urlencode($row['id_artigo']) . '&designacao_artigo=' . urlencode($row['designacao_artigo']) . '&descricao=' . urlencode($row['descricao']) . '">
        ...
        echo '<td><div class="delete">
                <a href="delete-artigo.php?id_artigo=' . urlencode($row['id_artigo']) . '" onclick="return confirm(\'Tem a certeza que deseja apagar este artigo?\')" style="color: red;">
    ...
    echo "<tr><td colspan='10'>Nenhum artigo encontrado</td></tr>";
}
...
$total_items_query = "SELECT COUNT(*) as total FROM artigos";
...
$sql = "SELECT DISTINCT * FROM artigos ORDER BY id_artigo ASC LIMIT $items_per_page OFFSET $offset";
$result = mysqli_query($conn, $sql) or die(mysqli_error($conn));
?>
...
<script>
      ...
      const list = new List('table-default', {
      	...
      	valueNames: [ 'sort-nome', 'sort-observacao']
      });
      })
    </script>
    <script>
